...
- (instancetype)init
{
    ...
    if (self != nil)
    {
        ...
        apiEnvironment.apiId = (int32_t)API_ID; // NSAssert(false, @"Please obtain your own apiId at https://my.telegam.org");
        ...
        {
            [_context setSeedAddressSetForDatacenterWithId:1
                                            seedAddressSet:[[MTDatacenterAddressSet alloc]
                                        initWithAddressList:@[[[MTDatacenterAddress alloc]
                                                initWithIp:API_DATASERVER_IP_DEBUG port:API_DATASERVER_PORT]]]];
            ...
            //]]];
            TGLog(@"In Testing environment, using API_DATASERVER_IP_DEBUG=%@", API_DATASERVER_IP_DEBUG);
        }
        else
        {
            [_context performBatchUpdates:^
            {
                [_context setSeedAddressSetForDatacenterWithId:1
                                                seedAddressSet:[[MTDatacenterAddressSet alloc]
                                            initWithAddressList:@[[[MTDatacenterAddress alloc]
                                                    initWithIp:API_DATASERVER_IP_RELEASE
                                                        port:API_DATASERVER_PORT]]]];
                TGLog(@"Not in Testing environment, using API_DATASERVER_IP_RELEASE=%@", API_DATASERVER_IP_RELEASE);
...
#else
    [[TGSession instance] updatePts:pts date:date seq:seq];
#endif
}
